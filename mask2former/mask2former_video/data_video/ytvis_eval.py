...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        """
        prediction = instances_to_coco_json_video(inputs, outputs, self._output_dir)
...
def instances_to_coco_json_video(inputs, outputs, output_dir=None):
    """
    ...
    # print(len(inputs[0]["file_names"]), len(scores), masks[0].shape)
    # video_name = inputs[0]["file_names"][0].split("/")[-2]
    # vis_name = output_dir.split("/")[-2]
    # vis_result_dir = f"vis_result/{vis_name}/"
    # os.makedirs(vis_result_dir, exist_ok=True)

    # vis_result_dir += video_name
    # torch.save(scores, vis_result_dir + "_scores")
    # torch.save(labels, vis_result_dir + "_labels")
    # torch.save(masks, vis_result_dir + "_masks")
    # print(vis_result_dir)
