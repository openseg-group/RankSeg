...
from torch.distributed import is_available, get_rank

from collections import OrderedDict

from timm.models.layers import drop_path, to_2tuple, trunc_normal_

from .helpers.beit import load_checkpoint
...
class BEiT(nn.Module):
    ...
    def __init__(
        ...
        self.build_unpooling(unpool_cfg, dict(embed_dims=embed_dim))
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.out_indices:
                xp = x[:, 1:]
                xp, reshaped = unpooler.call(xp)
                if not reshaped:
                    xp = reshape_as_aspect_ratio(xp, aspect_ratio)
