...
class VisionTransformer(BaseModule):
    ...
    def __init__(
        ...
        if with_cls_emb:
            self.cls_emb = nn.Parameter(torch.zeros(1, num_classes, embed_dims))
        else:
            self.cls_emb = None
        self.cls_emb_start_layer = cls_emb_start_layer
    ...
    def init_weights(self):
        if isinstance(self.pretrained, str):
            ...
            if self.cls_emb is not None:
                trunc_normal_init(self.cls_emb, std=0.02)
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            if self.cls_emb is not None and i == self.cls_emb_start_layer:
            ...
            if i in self.out_indices:
                if self.with_cls_token:
                    ...
                    out = x
                if self.cls_emb is not None and i >= self.cls_emb_start_layer:
                    cls_emb = out[:, -self.cls_emb.shape[1] :]
                    out = out[:, : -self.cls_emb.shape[1]]
                B, _, C = out.shape
