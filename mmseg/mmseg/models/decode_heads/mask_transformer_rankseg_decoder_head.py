...
class MaskTransformerRankSegDecoderHead(BaseDecodeHead):
    ...
    def __init__(
            ...
            img_cls_head=dict(type="RankSegEncoderHead"),
            **kwargs,
    ):
        ...
        if img_cls_head["type"] == "RankSegGlobalPoolingHead":
            ...
            self.img_cls_head = nn.Sequential(
                ...
                nn.Conv2d(d_encoder, n_cls, kernel_size=1),
            )
        else:
            self.img_cls_head = build_head(img_cls_head)
    ...
    def forward(self, x):
        ...
        if self.downsample:
            downsample_x = self.pooling(x)

        if (hasattr(self.img_cls_head, "share_embedding")
                and self.img_cls_head.share_embedding):
            img_pred = self.img_cls_head(
                (downsample_x, self.cls_emb.squeeze(0)))
        else:
            img_pred = self.img_cls_head(downsample_x)
